...
var wallet *argo.Wallet

func init() {
	var err error
	wallet, err = argo.NewWalletFromPath(privateKey, arNode)
	if err != nil {
		panic(err)
	}
}

func assemblyDataTx(bigData []byte, wallet *argo.Wallet, tags []types.Tag) (*types.Transaction, error) {
...
func Test_PostBigDataByChunks(t *testing.T) {
	...
	tags := []types.Tag{{Name: "Content-Type", Value: "application/pdf"}, {Name: "argo", Value: "testdata"}}
	tx, err := assemblyDataTx(bigData, wallet, tags)
	assert.NoError(t, err)
	t.Log("txHash: ", tx.ID)

	// uploader Transaction
	uploader, err := argo.CreateUploader(wallet.Client, tx, nil)
	assert.NoError(t, err)
...
func Test_RetryUploadDataByTxId(t *testing.T) {
	...
	tags := []types.Tag{{Name: "Content-Type", Value: "application/jpg"}, {Name: "argo", Value: "testdata"}}
	...
	uploader, err := argo.CreateUploader(wallet.Client, tx.ID, bigData)
	assert.NoError(t, err)
	assert.NoError(t, uploader.Once())
}
...
func Test_ContinueUploadDataByLastUploader(t *testing.T) {
	...
	tags := []types.Tag{{Name: "Content-Type", Value: "application/jpg"}, {Name: "argo", Value: "1.8mbPhoto"}}
	tx, err := assemblyDataTx(bigData, wallet, tags)
	assert.NoError(t, err)
	t.Log("txHash: ", tx.ID)
	...
	assert.NoError(t, err)
	lastUploader := &argo.TransactionUploader{}
	err = json.Unmarshal(uploaderBuf, lastUploader)
	assert.NoError(t, err)

	// new uploader object by last time uploader
	newUploader, err := argo.CreateUploader(wallet.Client, lastUploader.FormatSerializedUploader(), bigData)
	assert.NoError(t, err)
