...
func TestBundle_SendBundleTx(t *testing.T) {
	...
	w, err := argo.NewWalletFromPath(privateKey, arNode)
	if err != nil {
		panic(err)
	}
	...
	tags := []types.Tag{
		{Name: "argo", Value: "bundleTx"},
		{Name: "App-Version", Value: "2.0.0"},
	}

	item01, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 01"), 1, target, "", tags)
	assert.NoError(t, err)
	item02, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 02"), 1, target, "", tags)
	assert.NoError(t, err)
	item03, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 03"), 1, target, "", tags)
	assert.NoError(t, err)
	item04, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 04"), 1, target, "", tags)
	assert.NoError(t, err)
	item05, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 05"), 1, target, "", tags)
	assert.NoError(t, err)
	item06, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 06"), 1, target, "", tags)
	assert.NoError(t, err)
	item07, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 07"), 1, target, "", tags)
	assert.NoError(t, err)
	item08, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 08"), 1, target, "", tags)
	assert.NoError(t, err)
	item09, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 09"), 1, target, "", tags)
	assert.NoError(t, err)
	item10, err := w.CreateAndSignBundleItem([]byte("argo bundle tx 10"), 1, target, "", tags)
	assert.NoError(t, err)
...
func TestVerifyBundleItem(t *testing.T) {
	cli := argo.NewClient("https://arweave.net")
