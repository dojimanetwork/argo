...
```
go get github.com/dojimanetwork/argo
```
...
func main() {
	wallet, err := argo.NewWalletFromPath("./test-keyfile.json", "https://arweave.net")
	if err != nil {
		panic(err)
	}
...
```golang
arClient := argo.NewClient("https://arweave.net")

// if your network is not good, you can config http proxy
proxyUrl := "http://127.0.0.1:8001"
arClient := argo.NewClient("https://arweave.net", proxyUrl)
```
...
```golang
arWallet := argo.NewWalletFromPath("./keyfile.json")
...
[Threshold Signature Usage Guidelines](https://github.com/dojimanetwork/argo/wiki/argo--RSA-Threshold-Signature-Usage-Guidelines)    
...
l := 5
k := 3
keyShares, keyMeta, err := argo.CreateTcKeyPair(bitSize, k, l)
```
...
ts, err := argo.NewTcSign(keyMeta, exampleData)
...
w, err := argo.NewWalletFromPath("../example/testKey.json", arNode) // your wallet private key
...
uploader, err := argo.CreateUploader(w.Client, tx, nil)
if err != nil {
  return
}
...
uploader, err := argo.CreateUploader(wallet.Client, txId, bigData)
assert.NoError(t, err)
...
1. `argo` implemented creating,editing,reading and verifying bundles tx
