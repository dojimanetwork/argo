...
// TestCreateKeyPair Secret key creation and threshold signaturer
func TestCreateTcKeyPair(t *testing.T) {
	...
	bitSize := 4096 // If the values are 2048 and 4096, then the generation functions below will perform minute-level times, and we need 4096 bits as the maximum safety level for production environments.
	l := 100
	k := 70
	...
	fmt.Printf("key share%v", keyShares)
	fmt.Print(keyMeta)
